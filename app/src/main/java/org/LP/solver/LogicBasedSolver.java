...
public class LogicBasedSolver {
    ...
    static double[][] students; //Sorted students matrix based on budget low to high, then on max distance low to high
    static double[][] houses; //Sorted houses matrix based on price low to high, then distance low to high
    ...
    public double LogicBasedSolver(double[][] students, double[][] houses) {
        ...
        for (int i = 0; i < students.length; i++) {
            ...
            //If the Distance constraint is violated, check if the student can be placed in another house
                ...
                for (int j = 0; j < i; j++) {
                    if (students[j][0] >= houses[houseNumber][0] && students[j][1] >= houses[houseNumber][1] && students[i][1] >= houses[solution[j]][1] && violationDistance) { //One mistake here
